...
class ForecastingTrader:
    ...
    def forecast_spread_trading(self, X, Y, spread_test, spread_train, beta, predictions, lag,
        ...
        mu = spread_train_pct_change.mean()

        # ffill if applicable
        if lag == 1:
            pct_change_from_previous = predictions_pct_change
        else:
            pct_change_from_previous = predictions_pct_change = (((predictions_1 - spread_test.shift(1)) / abs(spread_test.shift(1))) * 100).fillna(0)
        ...
        position_ret_with_costs = trader.add_transaction_costs(df, beta=beta, comission_costs=0, market_impact=0, short_rental=0)
    ...
    def run_specific_model(self, pairs, model, path='models/', train_val_split='2017-01-01', lag=1,
    ...
    def test_specific_model(self, pairs, model, train_test_split='2018-01-01', lag=1,
